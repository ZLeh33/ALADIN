...
class QueryGenerator {
	...
	private generateOrderBy(joinInstruction: IJoinInstruction): IOrderByColumns {
		...
		const sampledColumnsPerTable = this.restructureColumnsPerTable({
			tables,
	...
	private generateWhereClause(columns: IColumns, constraintRange: Array<number>) {
		...
		const constraintColumns = Object.entries(sampledConstraintColumns).reduce(
			async (constraintColumns, [table, columns]) => {
	...
	private joinInstructionToColumns(joinInstruction: IJoinInstruction) {
		...
		const columns = selectedTables.reduce((columns, selectedTable) => {
			const columnsPerTable = Object.entries(tables[selectedTable].columns).map(([name, columnType]) => ({
				name,
				...columnType,
			}));
...
class SQLParser {
	public parse(
		{ columns, join, whereClause, groupBy, havingClause, orderBy, aliasDictionary }: IStructuredQuery,
		schema: string
	) {
...
const templatesPerLanguage: ITemplates = {
	de: {
		...
		constraintTemplate: {
			...
			endingPhrase: "gilt",
			LIKEOperatorFallback: {
				exclude: "eine beliebige Zeichenkette enthält",
				include: "eine leere Zeichenkette enthält",
			},
		},
		groupByTemplate: {
...
export const sqlQueryGenerator = async (taskDescription: SQLTaskDescription) => {
	...
	const dotDescription = loadERD(schema);
...
const loadERD = (schema: string) => {
	const erdFolder = path.join(__dirname, "ERDs");
	return fs.readFileSync(`${erdFolder}/${schema}.dot`, "utf8");
};
